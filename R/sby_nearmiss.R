...
#' @param sby_knn_hnsw_m Número inteiro positivo que define a conectividade do grafo HNSW quando `RcppHNSW` é usado. O padrão é `16L`. Valores maiores podem melhorar a fidelidade da vizinhança aproximada para o ranqueamento NearMiss, com maior custo de memória.
#' @param sby_knn_hnsw_ef Número inteiro positivo que define a quantidade dinâmica de candidatos avaliados na busca HNSW. O padrão é `200L`. Aumentar esse valor tende a produzir vizinhos aproximados mais confiáveis e reduz risco de retenção baseada em vizinhos subótimos, mas torna a execução mais lenta.
#'
#' @details
...
sby_nearmiss <- function(
  ...
  if(!is.numeric(sby_knn_under_k) || length(sby_knn_under_k) != 1L || is.na(sby_knn_under_k) || sby_knn_under_k < 1L){

    # Aborta quando o parametro NearMiss nao representa inteiro positivo
    sby_adanear_abort(
      sby_message = "'sby_knn_under_k' deve ser inteiro positivo"
    )
  }
