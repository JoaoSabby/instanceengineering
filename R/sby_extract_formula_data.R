...
sby_extract_formula_data <- function(sby_formula, sby_data){
  ...
  if(!all(sby_predictor_names %in% colnames(sby_data_frame))){
    ...
    sby_adanear_abort(
      sby_message = paste0("'sby_formula' deve referenciar colunas existentes em 'sby_data' preditores: ", paste0(sort(sby_predictor_names), collapse = ", "), " colunas dados: ", paste0(sort(colnames(sby_data_frame)), collapse = ", "))
    )
  }
